...
class Operator:
    ...
    def init_populate(self):
        ...
        self.pop.df_population = pd.DataFrame(
            self.pop.list_population, columns=cfg.DATA_FIELDS_VOCALS
        )
    ...
    def init_from_corpus(self, df_corpus, df_population):
        self.sd.logger.info("Initiating from corpus...")
        self.current_population += 1
        df_population["pop"] = self.current_population
    ...
    def operate(self, event_populating, event_umap_projection):
        while True:
            # df_tsne = self.tsne_project()  # temp
            event_umap_projection.clear()
            ...
            self.sd.logger.info(f"Selection threshold: {threshold}")
            while True:
